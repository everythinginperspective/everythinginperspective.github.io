...
def save_results(results, prefix='fetch_top_wikiarticles'):
    """Save results to both master and timestamped files."""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    master_file = f'{prefix}_master.json'
    timestamped_file = f'{prefix}_{timestamp}.json'
    for filepath in [master_file, timestamped_file]:
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
    return master_file, timestamped_file
...
def main():
    ...
    for i, lang in enumerate(wiki_langs, 1):
        print(f"  [{i}/{len(wiki_langs)}] {lang}...", end=' ', flush=True)
    ...
    # Final summary
    master_file, timestamped_file = save_results(results)
